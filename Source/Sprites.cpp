...
public:
    ...
    bool checkCollision(Player& player)
    {
      ...
      void collision(const std::vector<std::unique_ptr<Immovable>>& platforms) //check collision with Platforms and Gold objects for avoiding the platforms
    {
        for (const auto& platform : platforms)
        {
    ...
    void update(const std::vector<std::unique_ptr<Immovable>>& platforms, const sf::RenderWindow& window)        // Keep heart objects inside the window
    {
